...
class KYSProcessing(BaseProcessing):
    ...
    def __call__(self, data: TensorDict):
        ...
        for s in ['train', 'test']:
            ...
            crops_event = []
            for i in range(len(data[s + '_event_stack'][0])):
                crops_event_tem, _, _ = prutils.jittered_center_crop([data[s + '_event_stack'][j][i] for j in range(len(data[s + '_event_stack']))],
                                                               jittered_anno, data[s + '_anno'], self.search_area_factor, self.output_sz)
                crops_event.append(crops_event_tem)
            ...
            for i in range(len(crops_event)):
                temp1, _ = self.transform[s](image=crops_event[i], bbox=boxes, joint=False)
                data[s + '_event_stack'][i] = temp1
            # data[s + '_event_stack'] = data[s + '_event_stack'][0:len(data[s + '_event_stack'][0])]
