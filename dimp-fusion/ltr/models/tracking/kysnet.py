...
class KYSNet(nn.Module):
    ...
    def train_classifier(self, train_imgs, e, train_bb):
        ...
        train_event_feat1, train_event_feat2 = self.motion_attention_train(e[0].reshape(-1, *e[0].shape[-3:]),
                                                                           e[1].reshape(-1, *e[1].shape[-3:]),
                                                                           e[2].reshape(-1, *e[2].shape[-3:]))
    ...
    def test_features(self, im, e):
        # im = im.view(-1, *im.shape[-3:])
        test_event_feat1, test_event_feat2 = self.motion_attention_test(e[0].reshape(-1, *e[0].shape[-3:]),
                                                                         e[1].reshape(-1, *e[1].shape[-3:]),
                                                                         e[2].reshape(-1, *e[2].shape[-3:]))

        test_feat = self.extract_backbone_features(im)
        [x1, x2] = self.counter_guide_test(test_feat['layer2'], test_feat['layer3'], test_event_feat1,
                                            test_event_feat2)
        test_feat['layer2'] = x1
        test_feat['layer3'] = x2
        return test_feat
