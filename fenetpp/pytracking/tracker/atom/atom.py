...
class ATOM(BaseTracker):
    ...
    def initialize(self, image, event_img, info: dict) -> dict:
        ...
        x = self.generate_init_samples(im, event_im)
        # event_x = self.generate_init_samples(event_im)
    ...
    def track(self, image, event_img, info: dict = None) -> dict:
        ...
        # Convert image
        im = numpy_to_torch(image)
        # event_im = numpy_to_torch(event_img)
        event_im = [numpy_to_torch(event_img[0]), numpy_to_torch(event_img[1]), numpy_to_torch(event_img[2])]
        self.im = im    # For debugging only
        ...
        test_x = self.extract_processed_sample(im, event_im, self.pos, sample_scales, self.img_sample_sz)
    ...
    def extract_sample(self, im: torch.Tensor, event_im:torch.Tensor, pos: torch.Tensor, scales, sz: torch.Tensor):
        return self.params.features.extract(im, event_im, pos, scales, sz)[0]
    ...
    def extract_processed_sample(self, im: torch.Tensor, event_im:torch.Tensor, pos: torch.Tensor, scales, sz: torch.Tensor) -> (TensorList, TensorList):
        x = self.extract_sample(im, event_im, pos, scales, sz)
    ...
    def generate_init_samples(self, im: torch.Tensor, event_im: torch.Tensor) -> TensorList:
        ...
        init_samples = self.params.features.extract_transformed(im, event_im, self.pos, self.target_scale, aug_expansion_sz, self.transforms)
        # init_samples_e  = self.params.features.extract_transformed(event_im, self.pos, self.target_scale, aug_expansion_sz, self.transforms)
