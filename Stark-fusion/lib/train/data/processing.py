    ...
    def __call__(self, data: TensorDict):
        ...
        if self.transform['joint'] is not None:
            ...
            for i in range(len(data['template_events'][0])):
                data['template_events'][0][i], _, _ = self.transform['joint'](image=data['template_events'][0][i],
                                                                             bbox=data['template_anno'],  mask=data['template_masks'])
            ...
            for i in range(len(data['search_events'][0])):
                data['search_events'][0][i], _, _ = self.transform['joint'](image=data['search_events'][0][i],
                                                                             bbox=data['search_anno'],  mask=data['search_masks'])
        ...
        for s in ['template', 'search']:
            ...
            for i in range(len(data[s + '_events'][0])):
                crops_event_tem, _, _, _ = prutils.jittered_center_crop([data[s + '_events'][0][i]], jittered_anno, data[s + '_anno'], \
            ...
            for i in range(len(crops_event)):
                temp1, _, _, _ = self.transform[s](image=crops_event[i], bbox=boxes, att=att_mask, mask=mask_crops, joint=False)
