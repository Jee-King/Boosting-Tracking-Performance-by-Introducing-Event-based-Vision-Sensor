...
class LTRBDenseRegressionProcessing(BaseProcessing):
    ...
    def __call__(self, data: TensorDict):
        ...
        if self.transform['joint'] is not None:
            ...
            train_event_copy =list(map(list, zip(*data['train_events'])))
            for i in range(len(train_event_copy)):
                train_event_copy[i], _ = self.transform['joint'](image=train_event_copy[i], bbox=data['train_anno'])
            data['train_events'] = train_event_copy

            test_event_copy =list(map(list, zip(*data['test_events'])))
            for i in range(len(test_event_copy)):
                test_event_copy[i], _ = self.transform['joint'](image=test_event_copy[i], bbox=data['test_anno'], new_roll=False)
            data['test_events'] = test_event_copy
        ...
        for s in ['train', 'test']:
            ...
            crops, boxes = prutils.target_image_crop(data[s + '_images'], jittered_anno, data[s + '_anno'],
                                                     ...
                                                     max_scale_change=self.max_scale_change)
            crops_event = []
            for i in range(len(data[s + '_events'])):
                crops_event_tem, _ = prutils.target_image_crop(data[s + '_events'][i], jittered_anno, data[s + '_anno'], self.search_area_factor, \
                    self.output_sz, mode=self.crop_type,
                    max_scale_change=self.max_scale_change)
