...
class TrackPairSampler(SamplerBase):
    ...
    def __getitem__(self, item) -> dict:
        ...
        while self.data_filter(data1) or self.data_filter(data2):
            ...
            data1["event1"] = load_image(data1["event1"])            
            data1["event2"] = load_image(data1["event2"])
            data1["event3"] = load_image(data1["event3"])

            data2["image"] = load_image(data2["image"])
            data2["event1"] = load_image(data2["event1"])
            data2["event2"] = load_image(data2["event2"])
            data2["event3"] = load_image(data2["event3"])
            sample_try_num += 1
        sampled_data = dict(
            data1=data1,
            data2=data2,
    ...
    def _sample_track_frame_from_sequence(self, sequence_data) -> Dict:
        ...
        data_frame['image'] = sequence_data['image'][idx]
        data_frame['anno'] = sequence_data['anno'][idx]

        data_frame['event1'] = sequence_data['event'][idx*3]
        data_frame['event2'] = sequence_data['event'][idx*3+1]
        data_frame['event3'] = sequence_data['event'][idx*3+2]
    ...
    def _sample_track_pair_from_sequence(self, sequence_data: Dict,
        ...
        data1 = {}
        data2 = {}
        data1['image'] = sequence_data['image'][idx1]
        data2['image'] = sequence_data['image'][idx2]
        data1['anno'] = sequence_data['anno'][idx1]
        data2['anno'] = sequence_data['anno'][idx2]

        data1['event1'] = sequence_data['event'][idx1*3]
        data1['event2'] = sequence_data['event'][idx1*3+1]
        data1['event3'] = sequence_data['event'][idx1*3+2]

        data2['event1'] = sequence_data['event'][idx2*3]
        data2['event2'] = sequence_data['event'][idx2*3+1]
        data2['event3'] = sequence_data['event'][idx2*3+2]
