...
class PipelineTracker(object):
    ...
    def init(self, image: np.array,e1,e2,e3, box):
        ...
        box : np.array or List
            ...
            formate: (x, y, w, h)
        """
        self.pipeline.init(image,e1,e2,e3, box)

    def update(self, image: np.array,e1,e2,e3):
        """Perform tracking
        ...
        np.array
            ...
            formate: (x, y, w, h)
        """
        return self.pipeline.update(image,e1,e2,e3)

    def track(self, img_files: List,event_files, box, visualize: bool = False):
        ...
        for f, img_file in enumerate(img_files):
            ...
            e1 = cv2.imread(event_files[f*3],cv2.IMREAD_COLOR)
            e2 = cv2.imread(event_files[f*3+1],cv2.IMREAD_COLOR)
            e3 = cv2.imread(event_files[f*3+2],cv2.IMREAD_COLOR)
            start_time = time.time()
            if f == 0:
                self.init(image, e1, e2, e3, box)
            else:
                boxes[f, :] = self.update(image, e1, e2, e3,)
