...
class ConvLSTM_qkv(nn.Module):
    ...
    def __init__(self, input_dim, hidden_dim, kernel_size, num_layers,
        ...
        self.conv2 = nn.Sequential(nn.Conv2d(in_channels=32, out_channels=32, kernel_size=3, stride=2, padding=1),
                                   nn.BatchNorm2d(32),
                                   nn.ReLU(inplace=True))
        self.conv3 = nn.Sequential(nn.Conv2d(in_channels=32, out_channels=3, kernel_size=3, stride=4, padding=1),
                                   nn.BatchNorm2d(3),
                                   nn.ReLU(inplace=True))
        # self.conv3_cat = nn.Sequential(nn.Conv2d(in_channels=sum(self.hidden_dim), out_channels=128, kernel_size=1, padding=0),
        #                            nn.BatchNorm2d(128),
        #                            nn.ReLU(inplace=True))
        self.conv3_cat = nn.Sequential(nn.Conv2d(in_channels=self.hidden_dim[-1]*3, out_channels=1024, kernel_size=1, padding=0),
                                   nn.BatchNorm2d(1024),
                                   nn.ReLU(inplace=True))
