...
class ToMPnet(nn.Module):
    ...
    def forward(self, train_imgs, test_imgs, train_event_stack, test_event_stack, train_bb, *args, **kwargs):
        ...
        train_event_feat = self.motion_attention_train(
            train_event_stack[0].reshape(-1, *train_event_stack[0].shape[-3:]).cuda(), \
        ...
        test_event_feat = self.motion_attention_test(
            test_event_stack[0].reshape(-1, *test_event_stack[0].shape[-3:]).cuda(), \
            test_event_stack[1].reshape(-1, *test_event_stack[1].shape[-3:]).cuda(), \
            test_event_stack[2].reshape(-1, *test_event_stack[2].shape[-3:]).cuda())

        train_feat['layer3'] = self.counter_guide_train(train_feat['layer3'], train_event_feat)

        test_feat['layer3'] = self.counter_guide_test(test_feat['layer3'], test_event_feat)
