...
class DiMPnet(nn.Module):
    ...
    def __init__(self, feature_extractor, classifier, bb_regressor, classification_layer, bb_regressor_layer):
        ...
        self.motion_attention_train = backbones.ConvLSTM_qkv(input_dim=3,
                                                             hidden_dim=[32, 32],
                                                             kernel_size=(3, 3),
                                                             num_layers=2,
                                                             batch_first=True,
        ...
        self.motion_attention_test = backbones.ConvLSTM_qkv(input_dim=3,
                                                            hidden_dim=[32, 32],
                                                            kernel_size=(3, 3),
                                                            num_layers=2,
                                                            batch_first=True,
    ...
    def forward(self, train_imgs, test_imgs, train_events, test_events, train_bb, train_label, test_proposals, *args, **kwargs):
        ...
        train_event_feat1, train_event_feat2 = self.motion_attention_train(
            train_events[0].reshape(-1, *train_events[0].shape[-3:]).cuda(), \
            train_events[1].reshape(-1, *train_events[1].shape[-3:]).cuda(), \
            train_events[2].reshape(-1, *train_events[2].shape[-3:]).cuda())
        test_event_feat1, test_event_feat2 = self.motion_attention_test(
            test_events[0].reshape(-1, *test_events[0].shape[-3:]).cuda(), \
            test_events[1].reshape(-1, *test_events[1].shape[-3:]).cuda(), \
            test_events[2].reshape(-1, *test_events[2].shape[-3:]).cuda())

        [x1, x2] = self.counter_guide_train(train_feat['layer2'], train_feat['layer3'], train_event_feat1,
                                            train_event_feat2)
