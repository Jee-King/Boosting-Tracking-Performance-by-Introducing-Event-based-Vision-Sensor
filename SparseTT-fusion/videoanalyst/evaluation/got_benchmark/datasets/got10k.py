...
class GOT10k(object):
    ...
    def _build_cache_for_current_subset(self):
        ...
        for seq_name in tqdm(self.seq_names):
            seq_dir = os.path.join(root_dir, subset, seq_name)
            img_files,event_files, anno, meta = self.load_single_sequence_img_event(seq_dir)
    ...
    def load_single_sequence(self, seq_dir):
        ...
        event_files = sorted(glob.glob(os.path.join(seq_dir,'inter3_stack','*.jpg')))
        ...
        if self.return_meta or self.subset == "val":
            ...
            return img_files,event_files, anno, None
