...
class SiamFCppTracker(PipelineBase):
    ...
    def feature(self, im: np.array,e1,e2,e3, target_pos, target_sz, avg_chans=None):
        """Extract feature
        ...
        if avg_chans is None:
            ...
            avg_chans1 = np.mean(e1, axis=(0, 1))
            avg_chans2 = np.mean(e2, axis=(0, 1))
            avg_chans3 = np.mean(e3, axis=(0, 1))

        z_size = self._hyper_params['z_size']
        context_amount = self._hyper_params['context_amount']

        im_z_crop,im_z_crop1,im_z_crop2,im_z_crop3, _ = get_crop(
            im,e1,e2,e3,
            target_pos,
            target_sz,
            z_size,
            avg_chans=avg_chans,avg_chans1=avg_chans1,avg_chans2=avg_chans2,avg_chans3=avg_chans3,
        ...
        with torch.no_grad():
            ...
            data1 = imarray_to_tensor(im_z_crop1).to(self.device)
            data2 = imarray_to_tensor(im_z_crop2).to(self.device)
            data3 = imarray_to_tensor(im_z_crop3).to(self.device)
            features = self._model(data,data1,data2,data3, phase=phase)

        return features, im_z_crop, avg_chans,avg_chans1,avg_chans2,avg_chans3

    def init(self, im,e1,e2,e3, state):
        ...
        features, im_z_crop, avg_chans,avg_chans1,avg_chans2,avg_chans3 = self.feature(im,e1,e2,e3, target_pos, target_sz)
        ...
        self._state['avg_chans1'] = avg_chans1
        self._state['avg_chans2'] = avg_chans2
        self._state['avg_chans3'] = avg_chans3
    ...
    def track(self,
        ...
        if 'avg_chans' in kwargs:
            ...
            avg_chans1 = self._state['avg_chans1']
            avg_chans2 = self._state['avg_chans2']
            avg_chans3 = self._state['avg_chans3']
    
        z_size = self._hyper_params['z_size']
        x_size = self._hyper_params['x_size']
        context_amount = self._hyper_params['context_amount']
        phase_track = self._hyper_params['phase_track']
        im_x_crop,im_x_crop1,im_x_crop2,im_x_crop3, scale_x = get_crop(
            im_x,e1, e2, e3,
            target_pos,
            target_sz,
            z_size,
        ...
        with torch.no_grad():
            score, box, cls, ctr, extra = self._model(
                imarray_to_tensor(im_x_crop).to(self.device),imarray_to_tensor(im_x_crop1).to(self.device),imarray_to_tensor(im_x_crop2).to(self.device),imarray_to_tensor(im_x_crop3).to(self.device),
                *features,
                phase=phase_track)
    ...
    def update(self, im, e1, e2, e3,state=None):
        ...
        target_pos, target_sz = self.track(im, e1, e2, e3,
                                           target_pos_prior,
                                           target_sz_prior,
                                           features,
                                           update_state=True)
