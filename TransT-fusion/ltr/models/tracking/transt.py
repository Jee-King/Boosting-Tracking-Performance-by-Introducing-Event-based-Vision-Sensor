...
class TransT(nn.Module):
    ...
    def __init__(self, backbone, featurefusion_network, num_classes):
        ...
        self.qkv = backbones.ConvLSTM_qkv(input_dim=3,
                                          hidden_dim=[32,32],
                                           kernel_size=(3, 3),
                                           num_layers=2,
                                           batch_first=True,
    ...
    def forward(self, search, template, search_event, template_event):
        ...
        if not isinstance(search, NestedTensor):
            ...
            se1 = nested_tensor_from_tensor(search_event[0][0][0])
            se2 = nested_tensor_from_tensor(search_event[0][1][0])
            se3 = nested_tensor_from_tensor(search_event[0][2][0])
        if not isinstance(template, NestedTensor):
            template = nested_tensor_from_tensor(template)
            te1 = nested_tensor_from_tensor(template_event[0][0][0])
            te2 = nested_tensor_from_tensor(template_event[0][1][0])
            te3 = nested_tensor_from_tensor(template_event[0][2][0])

        feature_search, pos_search = self.backbone(search)
        feature_search_event = self.qkv(se1.tensors, se2.tensors, se3.tensors)
        fused_search_feature = self.counter_guide(feature_search[0].tensors, feature_search_event)
        feature_search[0].tensors = fused_search_feature

        feature_template, pos_template = self.backbone(template)
        feature_template_event = self.qkv(te1.tensors, te2.tensors, te3.tensors)
        fused_template_feature = self.counter_guide(feature_template[0].tensors, feature_template_event)
        feature_template[0].tensors = fused_template_feature
    ...
    def track(self, search, event):
        if not isinstance(search, NestedTensor):
            search = nested_tensor_from_tensor_2(search)
            e1 = nested_tensor_from_tensor_2(event[0])
            e2 = nested_tensor_from_tensor_2(event[1])
            e3 = nested_tensor_from_tensor_2(event[2])

        features_search, pos_search = self.backbone(search)
        xf = self.qkv(e1.tensors, e2.tensors, e3.tensors)
        fused_search_feature = self.counter_guide(features_search[0].tensors, xf)
        features_search[0].tensors = fused_search_feature
    ...
    def template(self, z, event):
        ...
        ef = self.qkv(e1.tensors, e2.tensors, e3.tensors)
        fused_search_feature = self.counter_guide(zf[0].tensors, ef)
        zf[0].tensors = fused_search_feature
        self.zf = zf
