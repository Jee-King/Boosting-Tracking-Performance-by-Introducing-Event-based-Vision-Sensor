...
class KYSActor(BaseActor):
    ...
    def __call__(self, data):
        ...
        e1 =  data['train_event_stack'][0].to(self.device)
        e2 = data['train_event_stack'][1].to(self.device)
        e3 = data['train_event_stack'][2].to(self.device)
        e = [e1 ,e2, e3]
        dimp_filters = self.net.train_classifier(train_images, e, train_anno)
        ...
        e1 = data['test_event_stack'][0][0, ...].to(self.device)
        e2 = data['test_event_stack'][1][0, ...].to(self.device)
        e3 = data['test_event_stack'][2][0, ...].to(self.device)
        e = [e1, e2, e3]
        backbone_feat_prev_all = self.net.test_features(test_image_cur, e)
        ...
        for i in range(1, sequence_length):
            ...
            e1 = data['test_event_stack'][0][i, ...].to(self.device)
            e2 = data['test_event_stack'][1][i, ...].to(self.device)
            e3 = data['test_event_stack'][2][i, ...].to(self.device)
            e = [e1, e2, e3]
            # backbone_feat_prev_all = self.net.test_features(test_image_cur, e)
            ...
            backbone_feat_cur_all = self.net.test_features(test_image_cur, e)
