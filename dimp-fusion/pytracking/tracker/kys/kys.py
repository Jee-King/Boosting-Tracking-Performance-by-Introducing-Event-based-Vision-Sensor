...
class KYS(BaseTracker):
    ...
    def extract_backbone_event_features(self, backbone_feat, event_im: torch.Tensor, pos: torch.Tensor, scales, sz: torch.Tensor):
        ...
        with torch.no_grad():
            output_event_features0, output_event_features1 = self.net.motion_attention_train(event_patches[0], \
                                                                                       event_patches[1],\
                                                                                       event_patches[2])
            tmp1, tmp2 = self.net.counter_guide_train(backbone_feat['layer2'],
    ...
    def generate_init_samples(self, im: torch.Tensor, event_im) -> TensorList:
        ...
        with torch.no_grad():
            ...
            output_event_features0, output_event_features1 = self.net.motion_attention_train(event_im_patches[0], \
                                                                                            event_im_patches[1], \
                                                                                            event_im_patches[2])
            tmp1, tmp2 = self.net.counter_guide_train(init_backbone_feat['layer2'],
