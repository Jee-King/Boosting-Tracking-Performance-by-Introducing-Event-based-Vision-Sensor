...
def get_crop(im,e1,e2,e3,
             target_pos,
             target_sz,
             z_size,
             x_size=None,
             avg_chans=(0, 0, 0),avg_chans1=(0, 0, 0),avg_chans2=(0, 0, 0),avg_chans3=(0, 0, 0),
             context_amount=0.5,
    ...
    if mask is not None:
        ...
        im_crop = func_get_subwindow(im, target_pos, output_size, round(s_crop),
                                     avg_chans)
        im_crop1 = func_get_subwindow(e1, target_pos, output_size, round(s_crop),
                                     avg_chans1)
        im_crop2 = func_get_subwindow(e2, target_pos, output_size, round(s_crop),
                                     avg_chans2)
        im_crop3 = func_get_subwindow(e3, target_pos, output_size, round(s_crop),
                                     avg_chans3)
        return im_crop,im_crop1,im_crop2,im_crop3, scale
