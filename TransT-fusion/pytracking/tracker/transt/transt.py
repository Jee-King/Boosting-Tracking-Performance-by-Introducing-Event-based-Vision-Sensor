...
class TransT(SiameseTracker):
    ...
    def initialize(self, image, event, info: dict) -> dict:
        ...
        e1_crop = self.get_subwindow(event[0], self.center_pos, cfg.TRACK.EXEMPLAR_SIZE, s_z, np.mean(event[0], axis=(0, 1)))
        e1_crop = e1_crop.float().mul(1.0 / 255.0).clamp(0.0, 1.0)
        e2_crop = self.get_subwindow(event[1], self.center_pos, cfg.TRACK.EXEMPLAR_SIZE, s_z, np.mean(event[1], axis=(0, 1)))
        e2_crop = e2_crop.float().mul(1.0 / 255.0).clamp(0.0, 1.0)
        e3_crop = self.get_subwindow(event[2], self.center_pos, cfg.TRACK.EXEMPLAR_SIZE, s_z, np.mean(event[2], axis=(0, 1)))
        e3_crop = e3_crop.float().mul(1.0 / 255.0).clamp(0.0, 1.0)
        ...
        e1_crop = tvisf.normalize(e1_crop, self.mean, self.std, self.inplace)
        e2_crop = tvisf.normalize(e2_crop, self.mean, self.std, self.inplace)
        e3_crop = tvisf.normalize(e3_crop, self.mean, self.std, self.inplace)

        self.net.template(z_crop, [e1_crop, e2_crop, e3_crop])
    ...
    def track(self, image, event, info: dict = None) -> dict:
        ...
        e1_crop = self.get_subwindow(event[0], self.center_pos, cfg.TRACK.INSTANCE_SIZE, round(s_x), np.mean(event[0], axis=(0, 1)))
        e1_crop = e1_crop.float().mul(1.0 / 255.0).clamp(0.0, 1.0)
        e2_crop = self.get_subwindow(event[1], self.center_pos, cfg.TRACK.INSTANCE_SIZE, round(s_x), np.mean(event[1], axis=(0, 1)))
        e2_crop = e2_crop.float().mul(1.0 / 255.0).clamp(0.0, 1.0)
        e3_crop = self.get_subwindow(event[2], self.center_pos, cfg.TRACK.INSTANCE_SIZE, round(s_x), np.mean(event[2], axis=(0, 1)))
        e3_crop = e3_crop.float().mul(1.0 / 255.0).clamp(0.0, 1.0)
        ...
        e2_crop = tvisf.normalize(e2_crop, self.mean, self.std, self.inplace)
        e3_crop = tvisf.normalize(e3_crop, self.mean, self.std, self.inplace)
        outputs = self.net.track(x_crop, [e1_crop, e2_crop, e3_crop])
