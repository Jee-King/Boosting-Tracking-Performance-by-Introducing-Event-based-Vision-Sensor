...
class STARKSActor(BaseActor):
    ...
    def forward_pass(self, data, run_box_head, run_cls_head):
        ...
        for i in range(self.settings.num_template):
            ...
            e1  = data['template_events'][0][0][0].transpose(0,1).cuda()
            e2 = data['template_events'][0][1][0].transpose(0,1).cuda()
            e3 = data['template_events'][0][2][0].transpose(0,1).cuda()

            feat_dict_list.append(self.net(img=NestedTensor(template_img_i, template_att_i), events=[e1,e2,e3], mode='backbone'))
        ...
        e1 = data['search_events'][0][0][0].transpose(0,1).cuda()
        e2 = data['search_events'][0][1][0].transpose(0,1).cuda()
        e3 = data['search_events'][0][2][0].transpose(0,1).cuda()
        feat_dict_list.append(self.net(img=NestedTensor(search_img, search_att), events=[e1,e2,e3], mode='backbone'))
