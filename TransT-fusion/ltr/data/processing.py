...
class TransTProcessing(BaseProcessing):
    ...
    def __call__(self, data: TensorDict):
        ...
        if self.transform['joint'] is not None:
            ...
            for i in range(len(data['search_events'][0])):
                data['search_events'][0][i], _ = self.transform['joint'](image=data['search_events'][0][i], bbox=data['search_anno'])
            ...
            for i in range(len(data['template_events'][0])):
                data['template_events'][0][i], _ = self.transform['joint'](image=data['template_events'][0][i], bbox=data['template_anno'], new_roll=False)
        ...
        for s in ['search', 'template']:
            ...
            if s == 'search':
                ...
                crops_event = []
                for i in range(len(data[s + '_events'][0])):
                    crops_event_tem, _, _ = prutils.jittered_center_crop(
                        [data[s + '_events'][0][i]], jittered_anno, data[s + '_anno'], self.search_area_factor, self.search_sz)
                ...
                for i in range(len(data[s + '_events'][0])):
                    crops_event_tem, _, _ = prutils.jittered_center_crop(
                        [data[s + '_events'][0][i]], jittered_anno, data[s + '_anno'], self.template_area_factor, self.temp_sz)
                    crops_event.append(crops_event_tem)
        ...
        # data['template_events'] = data['template_events'].squeeze()
        # data['search_events'] = data['search_events'].squeeze()
