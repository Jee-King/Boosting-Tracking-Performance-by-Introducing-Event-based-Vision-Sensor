...
class ConvLSTM_qkv(nn.Module):
    ...
    def __init__(self, input_dim, hidden_dim, kernel_size, num_layers,
        ...
        #                            nn.BatchNorm2d(128),
        #                            nn.ReLU(inplace=True))
        self.conv3_cat = nn.Sequential(nn.Conv2d(in_channels=self.hidden_dim[-1]*3, out_channels=256, kernel_size=1, padding=0),
                                   nn.BatchNorm2d(256),
                                   nn.ReLU(inplace=True))
...
if __name__ == '__main__':
    ...
    model = ConvLSTM_qkv(input_dim=channels,
                 hidden_dim=[32,32],
                 kernel_size=(3, 3),
                 num_layers=2,
                 batch_first=True,
    ...
    model = model.cuda()
    var1 = torch.FloatTensor(60, 3, 128, 128).cuda()
    var2 = torch.FloatTensor(60, 3, 128, 128).cuda()
    var3 = torch.FloatTensor(60, 3, 128, 128).cuda()
    low = model(var1, var2, var3)
    print(low.shape)
