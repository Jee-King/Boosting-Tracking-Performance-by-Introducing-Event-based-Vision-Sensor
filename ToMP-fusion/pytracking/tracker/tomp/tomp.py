...
class ToMP(BaseTracker):
    ...
    def extract_backbone_event_features(self, backbone_feat, event_im: torch.Tensor, pos: torch.Tensor, scales, sz: torch.Tensor):
        ...
        with torch.no_grad():
            output_event_features = self.net.motion_attention_test(event_patches[0], \
                                                                   event_patches[1],\
                                                                   event_patches[2])
            tmp = self.net.counter_guide_test(backbone_feat['layer3'], output_event_features)
            backbone_feat['layer3'] = tmp
    ...
    def generate_init_samples(self, im: torch.Tensor, event_im) -> TensorList:
        ...
        event_im_patches = []
        for event in event_im:
            temp_patch = sample_patch_transformed(event, self.init_sample_pos, self.init_sample_scale, aug_expansion_sz,
                                                  self.transforms)
            temp_patch = temp_patch / 255
            temp_patch -= torch.Tensor((0.485, 0.456, 0.406)).view(1, -1, 1, 1)
            temp_patch /= torch.Tensor((0.229, 0.224, 0.225)).view(1, -1, 1, 1)
        ...
        with torch.no_grad():
            ...
            output_event_features = self.net.motion_attention_test(event_im_patches[0], \
                                                                   event_im_patches[1], \
                                                                   event_im_patches[2])
            tmp = self.net.counter_guide_test(init_backbone_feat['layer3'], output_event_features)
            init_backbone_feat['layer3'] = tmp
